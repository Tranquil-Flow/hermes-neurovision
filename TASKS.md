...
- [ ] Task 7: Create events.py — VisionEvent dataclass + EventPoller. Write tests first.
- [ ] Task 8: Create sources/custom.py — JSONL file tailer for ~/.hermes/vision/events.jsonl. Write tests first.
- [ ] Task 9: Create sources/state_db.py — SQLite poller for ~/.hermes/state.db (sessions + messages tables). Write tests first.
- [ ] Task 10: Create sources/memories.py — Filesystem watcher for ~/.hermes/memories/. Write tests first.
- [ ] Task 11: Create sources/cron.py — Cron job status poller from ~/.hermes/cron/. Write tests first.
- [ ] Task 12: Create sources/aegis.py — Optional Aegis audit trail tailer (graceful if missing). Write tests first.
- [ ] Task 13: Create sources/hook_handler.py — Standalone gateway hook + HOOK.yaml. Must NOT import hermes_vision. Write tests first.
