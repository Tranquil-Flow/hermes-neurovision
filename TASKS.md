...
- [ ] Task 44: Add Shift+Left / Shift+Right as additional gallery nav bindings. [SONNET]
...
- [ ] Task 45: Create tune.py — TuneSettings dataclass + TuneOverlay. [SONNET]
  File: hermes_neurovision/tune.py
  TuneSettings dataclass with fields (all float, all default 1.0):
  ...
  Each param has: min, max, step, label.
  TuneOverlay: draws a centered modal (max 30w x 16h) when active.
  UI layout:
    ┌─── TUNER ─────────────────┐
    │                           │
    │ ▶ Burst scale       1.0   │
...
- [ ] Task 46: Wire TuneSettings into GalleryApp and all effect pathways. [SONNET]
  ...
  Apply TuneSettings in scene.py:
...
- [ ] Task 47: Create debug_panel.py — right-side structured diagnostic panel. [SONNET]
