"""Auto-launch hermes-vision in a new terminal window."""
...
def is_already_running() -> bool:
    """Check if hermes-vision is already running."""
    try:
        ...
        # Look for hermes-vision processes (excluding this one and grep)
        lines = result.stdout.split('\n')
        count = sum(1 for line in lines if 'hermes-vision' in line and 'grep' not in line and str(os.getpid()) not in line)
        return count > 0
    except Exception:
...
def launch_tmux(command: str) -> bool:
    ...
    try:
        subprocess.Popen(
            ["tmux", "new-window", "-n", "hermes-vision", command],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            start_new_session=True
        )
...
def auto_launch(command: str = "hermes-vision --auto-exit --logs") -> bool:
    """
    Auto-launch hermes-vision in a new terminal.
